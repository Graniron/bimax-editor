...
import { AlertCircleIcon, ArrowRight, GlobeIcon, Loader2Icon } from "lucide-react";
  ...
  return <div className="flex flex-col gap-2">
    ...
    <Button variant="outline" asChild className="h-auto items-start justify-start p-4 bg-background border rounded-none flex flex-col gap-2">
      <Link href={`/projects/${data._id}`} className="group">
        ...
        <span className="text-xs text-muted-foreground">{formatTimestamp(data.updateAt)}</span>
      </Link>
    </Button>
  </div>
}
...
const ProjectItem = ({ data }: { data: Doc<"projects">}) => {
  return <Link href={`/projects/${data._id}`} className="text-sm text-foreground/60 font-medium hover:text-foreground py-1 flex items-center justify-between w-full group">
    ...
    <span className="text-xs text-muted-foreground group-hover:text-foreground/60 transition-colors">{formatTimestamp(data.updateAt)}</span>
  </Link>
};
