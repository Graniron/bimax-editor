...
export const Navbar = ({ projectId }: { projectId: Id<"projects"> }) => {
  ...
  return <nav className="flex justify-between items-center gap-x-2 p-2 bg-sidebar border-b">
            ...
            <BreadcrumbLink className="flex items-center gap-1.5" asChild href="/projects">
              <Button variant="ghost" className="w-fit! p-1.5! h-7!" asChild>
                <Link href="/">
                  ...
                  <span className={cn("text-small font-medium", font.className)}>Bimax Editor</span>
                </Link>
              </Button>
            </BreadcrumbLink>
          </BreadcrumbItem>
      ...
      {project?.importStatus === "importing" ? (
        ...
        project?.updateAt && (
          <Tooltip>
            ...
            <TooltipContent>Saved {formatDistanceToNow(project?.updateAt, { addSuffix: true })}</TooltipContent>
          </Tooltip>
        )
      )}
    </div>
